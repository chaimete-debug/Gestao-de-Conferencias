window.UI = {
  ...
  status(value){const v=String(value||'').toUpperCase();const ok=['PAGO','CONFIRMADO','CONFIRMADA','PRESENTE','ACTIVO','CONCLUIDA'];const warn=['SUBMETIDO','SUBMETIDA','PENDENTE','PARCIAL','PROGRAMADA','EM_VERIFICACAO'];const danger=['REJEITADO','CANCELADA','RECUSADA','INACTIVO'];const cls=ok.includes(v)?'ok':warn.includes(v)?'warn':danger.includes(v)?'danger':'neutral';return `<span class="status ${cls}">${this.escape(String(value||'—').replaceAll('_',' '))}</span>`;},
