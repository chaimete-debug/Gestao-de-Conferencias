window.UI = {
  ...
  status(value){const v=String(value||'').toUpperCase();const ok=['PAGO','CONFIRMADO','CONFIRMADA','PRESENTE','ACTIVO','CONCLUIDA','CONCLUIDO','HOSPEDADA','EMBARCADA','EMITIDO','SAIDA'];const warn=['SUBMETIDO','SUBMETIDA','PENDENTE','PARCIAL','PROGRAMADA','PROGRAMADO','PLANEADA','ATRIBUIDA','EM_VERIFICACAO','EM_SERVICO','EM_CURSO','RASCUNHO','PREPARADA','AGENDADA','EM_ENVIO'];const danger=['REJEITADO','CANCELADA','CANCELADO','RECUSADA','INACTIVO','SUBSTITUIDO'];const cls=ok.includes(v)?'ok':warn.includes(v)?'warn':danger.includes(v)?'danger':'neutral';return `<span class="status ${cls}">${this.escape(String(value||'—').replaceAll('_',' '))}</span>`;},
